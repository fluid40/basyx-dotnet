...
{
    public static class SubmodelDiscoveryExtensions
    {
        ...
        private static ISubmodelRegistryInterface submodelRegistry;

        public const string SUBMODEL_ID = "sm.id";
        public const string SUBMODEL_IDSHORT = "sm.idShort";
        public const string SUBMODEL_ENDPOINT = "sm.endpoint";
        ...
        public static void StartDiscovery(this ISubmodelRegistryInterface registry)
        {
            submodelRegistry = registry;
        ...
        private static void DiscoveryServer_ServiceInstanceDiscovered(object sender, ServiceInstanceEventArgs e)
        {
            try
            {
                ISubmodelDescriptor submodelDescriptor = null;
                if (e?.TxtRecords?.Count > 0)
                {
                    string aasIdRecord = e.TxtRecords.Where(s => s.StartsWith(SUBMODEL_ID))?.FirstOrDefault();
                    if (!string.IsNullOrEmpty(aasIdRecord))
                    {
                        ...
                        {
                            var retrieved = submodelRegistry.RetrieveSubmodelRegistration(splittedRecord[1]);
                            if (retrieved.SuccessAndContent)
                                return;
                            else
                            {
                                List<string> endpointRecords = e.TxtRecords.Where(s => s.StartsWith(SUBMODEL_ENDPOINT))?.ToList();
                                if(endpointRecords?.Count > 0)
                                    AssembleDescriptor(ref submodelDescriptor, endpointRecords, e.Servers);
                            }
                        }
                    }
                    else
                ...
                if (submodelDescriptor != null && submodelRegistry != null)
                {
                    var registeredResult = submodelRegistry.CreateSubmodelRegistration(submodelDescriptor);
                    if (registeredResult.Success)
                        logger.LogInformation($"Successfully registered Asset Administration Shell with {submodelDescriptor.Id} at registry");
                    else
                        registeredResult.LogResult(logger, LogLevel.Error, $"Could not register Asset Administration Shell with {submodelDescriptor.Id} at registry");
                }
            }
            catch (Exception exc)
            {
        ...
        private static void AssembleDescriptor(ref ISubmodelDescriptor smDescriptor, List<string> endpointRecords, List<Server> servers)
        {
            ...
            if (smDescriptor == null)
            {
                foreach (string endpointRecord in endpointRecords)
                {
                    ...
                    if (splittedEndpoint.Length == 2 && (splittedEndpoint[0].ToLower().Contains("http") || splittedEndpoint[0].ToLower().Contains("https")))
                    {
                        ...
                        if (endpointValid)
                        {
                            using (var client = new SubmodelHttpClient(endpoint, SimpleHttpClient.DEFAULT_HTTP_CLIENT_HANDLER))
                            {
                                var retrieveDescriptor = client.RetrieveSubmodelDescriptor();
                                if (retrieveDescriptor.SuccessAndContent)
                                {
                                    smDescriptor = retrieveDescriptor.Entity;
                                    break;
                                }
                                else
                                    continue;
            ...
            if(smDescriptor != null)
            {
                foreach (var aasEndpoint in smDescriptor.Endpoints.ToList())
                {
                    Uri endpoint = new Uri(aasEndpoint.ProtocolInformation.EndpointAddress);
                    bool endpointValid = CheckEndpoint(endpoint, servers);
                    if (!endpointValid)
                        smDescriptor.DeleteEndpoint(aasEndpoint);
                }
            }
        }
        ...
        private static void DiscoveryServer_ServiceInstanceShutdown(object sender, ServiceInstanceEventArgs e)
        {
            try
            {
                if (submodelRegistry != null && e.TxtRecords?.Count > 0)
                {
                    string aasIdKeyValue = e.TxtRecords.FirstOrDefault(t => t.StartsWith(SUBMODEL_ID + KEY_VALUE_SEPERATOR));
                    if (!string.IsNullOrEmpty(aasIdKeyValue))
                    {
                        ...
                        {
                            if (splittedItem[0] == SUBMODEL_ID)
                            {
                                var deletedResult = submodelRegistry.DeleteSubmodelRegistration(splittedItem[1]);
        ...
        public static void StopDiscovery(this ISubmodelRegistryInterface registryHttpServer)
        {
            discoveryServer.ServiceInstanceDiscovered -= DiscoveryServer_ServiceInstanceDiscovered;
            discoveryServer.ServiceInstanceShutdown -= DiscoveryServer_ServiceInstanceShutdown;
        ...
        public static void StartDiscovery(this ISubmodelServiceProvider serviceProvider)
        {
        ...
        public static void StartDiscovery(this ISubmodelServiceProvider serviceProvider, int port, IEnumerable<IPAddress> iPAddresses)
        {
            discoveryClient = new DiscoveryClient(serviceProvider.ServiceDescriptor.IdShort, (ushort)port, ServiceTypes.SM_SERVICE_TYPE, iPAddresses);
            discoveryClient.AddProperty(SUBMODEL_ID, serviceProvider.ServiceDescriptor.Id);
            discoveryClient.AddProperty(SUBMODEL_IDSHORT, serviceProvider.ServiceDescriptor.IdShort);
            for (int i = 0; i < serviceProvider.ServiceDescriptor.Endpoints.Count(); i++)
            {
                ...
                discoveryClient.AddProperty(SUBMODEL_ENDPOINT + "." + endpoint.ProtocolInformation.EndpointProtocol + "." + i, endpoint.ProtocolInformation.EndpointAddress);
            }
        ...
        public static void StartDiscovery(this ISubmodelRepositoryServiceProvider serviceProvider)
        {
            List<IPAddress> ipAddresses = new List<IPAddress>();
            int port = -1;
            foreach (var endpoint in serviceProvider.ServiceDescriptor.Endpoints)
        ...
        /// <param name="port">The port to advertise</param>
        /// <param name="iPAddresses">A list of IP-addresses to advertise, if empty uses locally dicoverable multicast IP addresses</param>
        public static void StartDiscovery(this ISubmodelRepositoryServiceProvider serviceProvider, int port, IEnumerable<IPAddress> iPAddresses)
        {
            foreach (var aasDescriptor in serviceProvider.ServiceDescriptor.SubmodelDescriptors)
            {
                var discoveryClient = new DiscoveryClient(aasDescriptor.IdShort, (ushort)port, ServiceTypes.AAS_SERVICE_TYPE, iPAddresses);
                discoveryClient.AddProperty(SUBMODEL_ID, aasDescriptor.Id);
                discoveryClient.AddProperty(SUBMODEL_IDSHORT, aasDescriptor.IdShort);
        ...
        public static void StopDiscovery(this ISubmodelServiceProvider serviceProvider)
        {
            discoveryClient.Stop();
        }

        public static void StopDiscovery(this ISubmodelRepositoryServiceProvider serviceProvider)
        {
